...
def get_sum_of_incidents_for_categories(time, cities=None):
    # return number of incidents for each category in each city from given time
    logger.info("Getting incidents in a day data...")
    ...
    df_incidents = df_incidents.filter((time >= col("start")) & (time <= col("end")) | col("end").isNull())
    # do not count the same incident few times:
...
if __name__ == "__main__":
    ...
    get_sum_of_incidents_for_categories(start_time, ["Warsaw", "Zagreb"])
